...
export function DateTimePicker({ date, setDate, label, required, disabled = false }: DateTimePickerProps) {
    ...
    return (
        <div className="relative" ref={containerRef}>
            <div
                ...
                className={`
                    w-full px-4 py-3 bg-gray-50 dark:bg-white/5 border rounded-xl flex items-center justify-between transition-all
                    ${disabled ? 'cursor-not-allowed opacity-70 border-gray-100 dark:border-white/5' : 'cursor-pointer border-gray-200 dark:border-white/10 hover:border-gray-300 dark:hover:border-white/20'}
                    ${isOpen && !disabled ? 'ring-2 ring-black dark:ring-white border-transparent bg-white dark:bg-[#111]' : ''}
                `}
            >
