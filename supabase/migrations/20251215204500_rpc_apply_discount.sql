-- Securely apply a discount to a reservation
-- Wraps logic in a SECURITY DEFINER function to bypass RLS for guest users
...
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER -- Runs as the function creator (postgres/admin), bypassing default RLS
SET search_path = public, gatepass -- Secure search path
AS $$
DECLARE
    v_discount RECORD;
...
BEGIN
    ...
    -- 3. Check Limits (Basic check, concurrency might still need locking but acceptable for now)
