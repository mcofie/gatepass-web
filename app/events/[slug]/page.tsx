...
export default async function EventPage({ params }: PageProps) {
    ...
    // Fetch tiers (only visible ones for public view)
    ...
    return (
        <div className="h-[100dvh] w-full bg-black relative overflow-hidden">
            ...
            <div className="absolute bottom-6 left-8 z-10 flex items-center gap-4 pointer-events-auto">
                {formatWebsiteUrl(event.social_website || event.organizers?.website) && (
                    <a href={formatWebsiteUrl(event.social_website || event.organizers?.website)!} target="_blank" rel="noopener noreferrer" className="text-white/60 hover:text-white transition-colors">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" /></svg>
                    </a>
                )}
                {formatInstagramUrl(event.social_instagram || event.organizers?.instagram) && (
                    <a href={formatInstagramUrl(event.social_instagram || event.organizers?.instagram)!} target="_blank" rel="noopener noreferrer" className="text-white/60 hover:text-white transition-colors">
                ...
                {formatTwitterUrl(event.social_twitter || event.organizers?.twitter) && (
                    <a href={formatTwitterUrl(event.social_twitter || event.organizers?.twitter)!} target="_blank" rel="noopener noreferrer" className="text-white/60 hover:text-white transition-colors">
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>
                    </a>
                )}
                {formatFacebookUrl(event.social_facebook || event.organizers?.facebook) && (
                    <a href={formatFacebookUrl(event.social_facebook || event.organizers?.facebook)!} target="_blank" rel="noopener noreferrer" className="text-white/60 hover:text-white transition-colors">
