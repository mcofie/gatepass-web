...
// Public payment callback for guest instalment portal
// Redirects back to the guest detail page or shows success
    ...
    useEffect(() => {
        const processCallback = async () => {
            ...
            try {
                ...
                // Detail page is one level up from /payment-callback
                const detailUrl = pathname.replace('/payment-callback', '')
        ...
        processCallback()
    }, [searchParams, router, paramsPromise, pathname])
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-3xl p-8 max-w-md w-full shadow-xl border border-gray-100 dark:border-zinc-800 text-center space-y-6">
                ...
                {status === 'completed' && (
                    <>
                        ...
                        <p className="text-xs text-gray-400">Redirecting to your plan details...</p>
                        <button
                            onClick={() => router.push(pathname.replace('/payment-callback', ''))}
...
export default function GuestInstalmentPaymentCallbackPage({ params }: { params: Promise<{ id: string }> }) {
    return (
        <Suspense fallback={
            <div className="min-h-screen bg-gray-50 dark:bg-zinc-950 flex items-center justify-center p-4">
                <div className="bg-white dark:bg-zinc-900 rounded-3xl p-8 max-w-md w-full shadow-xl border border-gray-100 dark:border-zinc-800 text-center space-y-6">
